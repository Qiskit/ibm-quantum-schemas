...
def test_data_bin_model():
    ...
    bit_array1 = BitArrayModel.from_numpy(np.array([0, 1, 2, 3, 255], dtype=np.uint8), num_bits=8)
    bit_array2 = BitArrayModel.from_numpy(np.array([1, 2, 3, 4], dtype=np.uint8), num_bits=4)
...
def test_sampler_pub_result_model():
    ...
    bit_array = BitArrayModel.from_numpy(np.array([0, 1, 2, 3, 255], dtype=np.uint8), num_bits=8)
    data_bin = DataBinModel(shape=[1, 2], field_names=["meas"], fields={"meas": bit_array})

    pub_result = SamplerPubResultModel(
        data=data_bin, metadata={"_pub_shots": 1024, "_meas_type": "classified"}
    )
...
def test_metadata_models():
    ...
    chunk_span = ChunkSpan(
        start=now, stop=now + datetime.timedelta(seconds=5.1), parts=[chunk_part]
    )
...
def test_sampler_result_model():
    ...
    bit_array = BitArrayModel.from_numpy(np.array([0, 1, 2, 3, 255], dtype=np.uint8), num_bits=8)
    data_bin = DataBinModel(shape=[1, 2], field_names=["meas"], fields={"meas": bit_array})
    pub_result = SamplerPubResultModel(data=data_bin, metadata={})

    now = datetime.datetime.now()
    chunk_span = ChunkSpan(
        start=now, stop=now + datetime.timedelta(seconds=5.1), parts=[ChunkPart(idx_item=0, size=2)]
    )

    result = SamplerResultModel(
        pub_results=[pub_result], metadata={"chunk_timing": [chunk_span.model_dump()]}
    )
...
def test_params_model_initialization(qpy_version):
    ...
    options = OptionsModel(
        default_shots=4096,
        execution=ExecutionOptionsModel(init_qubits=True, meas_type="classified"),
...
def test_result_models_validation():
    ...
    with pytest.raises(ValueError):
        BitArrayModel(
            array="eJyb7BfqGxDJyFDGUK2eklqcXKRupaBuk2ahYGBgYAIA5OwGxA==", num_bits="not_an_int"
        )  # type: ignore
...
def test_runtime_encoder_compatibility():
    ...
    data_bin_value = {"shape": [1, 2], "field_names": ["meas"], "fields": {"meas": bit_array}}
